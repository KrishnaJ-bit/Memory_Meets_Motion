...
| 36 | 2026-08-04 14:02 PDT | 7 | All four sponsors | Public Vercel demo | Deployed Oto.ai's browser demo to Vercel with safe serverless SSE endpoints for the simulated handoff path and LaserData instance metadata for `starter-xipXm`; public smoke tests passed for page load, `prepare-stream`, `approve-stream`, and `activity-events`. | Production URL `https://oto-ai-memory-meets-motion.vercel.app`; deployment `dpl_H1DqPmWfG6xDbLfQUf9dfVXDScvD`; smoke: `/` 200, `/api/autopilot/prepare-stream` 200 with `event: ready` + LaserData host, `/api/autopilot/approve-stream` 200 with `event: done`, `/api/activity-events` 200; `vercel inspect` -> Ready, aliases set | done (public simulated path) |
| | | | | | | | |
